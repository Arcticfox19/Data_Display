...
from applications.models.admin_mail import VQ
...
def data():
    if current_user.username == current_app.config.get("SUPERADMIN"):
        ...
        PhoneNumber = str_escape(request.args.get('PhoneNumber'))
        # 查询参数构造
        mf = ModelFilter()
        if Name:
            mf.contains(field_name="Name", value=Name)
        if PhoneNumber:
            mf.contains(field_name="PhoneNumber", value=PhoneNumber)
        # orm查询
        # 使用分页获取data需要.items
        mail = VQ.query.filter(mf.get_filter(VQ)).layui_paginate()
        count = mail.total
        # 返回api
