...
bp = Blueprint('user', __name__, url_prefix='/vq')
...
def main():
    return render_template('system/vq/main.html')
...
def data():
    ...
    # 方式二： 使用原生sql查詢
    # 参考： https://blog.csdn.net/qq_34663267/article/details/109120033
    sql_query = """
        SELECT u.*, d.*
        FROM admin_user u
    ...
    """
    if real_name:
        sql_query += f" AND u.realname = {real_name}"
    if username:
        sql_query += f" AND u.username = {username}"
    if dept_id:
        sql_query += f" AND u.dept_id = {dept_id}"
